...
function Map:initialize(mapData)
    ...
    self.tileWidth = mapData.tilesets[1].tilewidth
    self.tileHeight = mapData.tilesets[1].tileheight
    self.tileSpacing = mapData.tilesets[1].spacing
...
function Map:render(batch, view_x, view_y, view_width, view_height)
    ...
    -- We ignore layers for this experiment, there's only one.
    local gameMap = self.layers[1].data
...
function love.load()
    ...
    gameResources.images.minimap = love.graphics.newImage('resources/mini-map.png')
    gameResources.quads.minimap = love.graphics.newQuad(0, 0,
        gameResources.images.minimap:getWidth(), gameResources.images.minimap:getHeight(), gameResources.images.minimap)
...
function love.draw()
    ...
    -- Render the minimap.
    --
    -- The map and minimap are both square, so we only have to keep track of
    -- one delta factor.
    local minimap_x = love.graphics.getWidth() - gameResources.images.minimap:getWidth()
    love.graphics.setColor(1, 1, 1, 1)
    love.graphics.draw(gameResources.images.minimap, gameResources.quads.minimap, minimap_x, 0)

    local size_delta = gameResources.images.minimap:getWidth() / gameResources.map.mapWidth
    local highlight_x = minimap_x + gameState.view_x * size_delta
    local highlight_y = gameState.view_y * size_delta
    local highlight_w = gameState.view_width * size_delta
    local highlight_h = gameState.view_height * size_delta
    love.graphics.setColor(1, 0, 1, 1)
    love.graphics.rectangle('line', highlight_x, highlight_y, highlight_w, highlight_h)
