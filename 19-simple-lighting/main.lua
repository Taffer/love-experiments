...
local function draw_shadows(x, y, range, alpha)
    love.graphics.setColor(0, 0, 0, alpha)

    for row = 0, math.floor(love.graphics.getHeight() / 32) do
        for column = 0, math.floor(love.graphics.getWidth() / 32) do
            local centre_x = column * 32 + 16
            local centre_y = row * 32 + 16

            local dx = x - centre_x
            local dy = y - centre_y

            local distance = math.floor(math.sqrt(dx * dx + dy * dy) / 32)

            if distance > range then
                -- Draw a shadow.
                love.graphics.rectangle('fill', column * 32, row * 32, 32, 32)
            end
        end
    end
end
...
function love.draw()
    ...
    -- Draw tile-based shadows centred on Sara's location. We use the middle of
    -- her tile instead of her actual location for cleaner output.
    draw_shadows(tile_x + tile_w / 2, tile_y + tile_h / 2, 5, 0.75)

    love.graphics.setColor(0, 0, 0, 1)
    love.graphics.setFont(gameResources.fonts.mono)
    print_shadow('Use the gamepad, WASD, or arrow keys to walk.', 10, 10)
end
