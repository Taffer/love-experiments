...
gameState = {
    ...
    blocks = {}, -- Columns that get in your way.
...
function love.load()
    ...
    gameResources.images.column = love.graphics.newImage('resources/col.png')
    ...
    gameResources.quads.column = love.graphics.newQuad(0, 0, gameResources.images.column:getWidth(),
        gameResources.images.column:getHeight(), gameResources.images.column)
    ...
    for y = 0, 720, TILE_SIZE do -- luacheck: pop
        for x = 0, (1280 - TILE_SIZE), TILE_SIZE do
    ...
    -- Define some blocks, aligned along tile boundaries. If one happens to
    -- spawn on top of Sara, she probably won't be able to move off of it.
    for i = 1, 10 do
        local block_x = math.random(gameState.screen_width - TILE_SIZE)
        local block_y = math.random(gameState.screen_height - TILE_SIZE)
        block_x = block_x - block_x % TILE_SIZE
        block_y = block_y - block_y % TILE_SIZE
        gameState.blocks[i] = {block_x, block_y}
    end
end
...
function love.draw()
    ...
    for _, block in ipairs(gameState.blocks) do
        ...
        love.graphics.rectangle('line', block[1], block[2], TILE_SIZE, TILE_SIZE)

        love.graphics.setColor(1, 1, 1, 1)
        love.graphics.draw(gameResources.images.column, gameResources.quads.column, block[1], block[2])
    end
...
function love.update(dt)
    ...
    -- Are we blocked by any blocks?
    local sara_x = math.floor((gameState.sara_x + gameState.sara.feet_x) / TILE_SIZE) * TILE_SIZE
    local sara_y = math.floor((gameState.sara_y + gameState.sara.feet_y) / TILE_SIZE) * TILE_SIZE

    local blocked = false
    for _, block in ipairs(gameState.blocks) do
        if sara_x == block[1] and sara_y == block[2] then
            blocked = true
        end
    end
    if blocked then
        gameState.sara_x = orig_x
        gameState.sara_y = orig_y
    end
